...
class GameViewController: UIViewController {
    ...
    @IBAction func startGameButton(_ sender: Any) {
        ...
        if(self.myModelRef!.fastestGameSoFar != Int.max){
            fastestGameLabel.text = "Fastest Game: \(self.myModelRef!.fastestGameSoFar) Attempts"
        }
        else{
    ...
    @IBAction func guessButton(_ sender: Any) {
        self.appDelegate = UIApplication.shared.delegate as? AppDelegate
        self.myModelRef = self.appDelegate?.theModel
        self.myModelRef?.guessed = true
        var guess:String = guessLabel1.text! + guessLabel2.text! + guessLabel3.text!
        self.myModelRef!.prevAttempt = guess
        lastGuessLabel.text = guess
        var correctness: [Bool] = self.myModelRef!.processGuess(pGuess: guess)
        attemptsThisGameLabel.text = String(self.myModelRef!.attemptsThisGame)
        print(self.myModelRef!.words[self.myModelRef!.lCurrentWordIndex!])
        print(correctness)
        ...
        correctLettersLabel.text = String(self.myModelRef!.correctLastAttempt)
...
} // end of class GameViewController: UIGameViewController
